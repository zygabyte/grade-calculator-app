...
function readSchoolsResponse(data){
    ...
    if (data.status) {
        data.data.forEach((school, i) => {
            ...
            row += '<td><a href="#" title="View" class="btn btn-primary btn-xs" onclick="viewSchoolClick(\'\' + school.id + \'\')"><i class="fa fa-eye"></i></a> | <button type="button" title="Edit" class="btn btn-success btn-xs" onclick="editSchoolClick(\'' + school.id + '\')"><i class="fa fa-pencil"></i></button> | <a href="#deleteSchoolModal" title="Delete" data-toggle="modal" class="btn btn-danger btn-xs" onclick="deleteSchoolClick(\'' + school.id + '\')"><i class="fa fa-trash-o"></i></a></td>';
            row += '</tr>';
