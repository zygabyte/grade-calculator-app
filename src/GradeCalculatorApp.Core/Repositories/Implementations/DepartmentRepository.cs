...
namespace GradeCalculatorApp.Core.Repositories.Implementations
{
    public class DepartmentRepository : IDepartmentRepository
    {
        ...
        public IEnumerable<Department> ReadDepartments(bool takeAll = true, int count = 1000)
        {
            try
            {
                return takeAll 
                    ...
                    : _gradeCalculatorContext.Departments.Where(x => !x.IsDeleted && x.IsActive).Take(count);
            }
            catch (Exception e)
            {
        ...
        public Department ReadDepartment(long departmentId)
        {
            try
            {
                return departmentId > 0 ? _gradeCalculatorContext.Departments.FirstOrDefault(x => !x.IsDeleted && x.IsActive) : null;
            }
            catch (Exception e)
            {
                return null;
        ...
        public bool UpdateDepartment(long departmentId, Department department)
        {
            try
            {
                ...
                if (department == null || currentDepartment == null) return false;
