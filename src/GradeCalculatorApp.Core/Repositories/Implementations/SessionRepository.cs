...
namespace GradeCalculatorApp.Core.Repositories.Implementations
{
    public class SessionRepository : ISessionRepository
    {
        ...
        public Session ReadSession(long sessionId)
        {
            try
            {
                return sessionId > 0 ? _gradeCalculatorContext.Sessions.FirstOrDefault(x => !x.IsDeleted && x.IsActive) : null;
            }
            catch (Exception e)
            {
                return null;
        ...
        public bool DeleteSession(long sessionId)
        {
            try
            {
                var session = _gradeCalculatorContext.Sessions.FirstOrDefault(x => !x.IsDeleted && x.IsActive);
        ...
        public bool UpdateSession(long sessionId, Session session)
        {
            try
            {
                var currentSession = _gradeCalculatorContext.Sessions.FirstOrDefault(x => !x.IsDeleted && x.IsActive);

                if (session == null || currentSession == null) return false;
